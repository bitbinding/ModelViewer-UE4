...
std::string ModelLoader::ToStdStringPath(const FString& fstr) {
#ifdef _WIN32
...
FString ModelLoader::FromStdStringPath(const std::string& str) {
#ifdef _WIN32
...
FString ModelLoader::FromStdStringPath(const std::string& str, bool isGBK) {
	if (isGBK) {
		return UTF8_TO_TCHAR(EncodeTransfer::GBKToUTF8(str).c_str());
	}
	else {
...
void ModelLoader::LoadSingleMesh(UProceduralMeshComponent* meshComponent, const FString& modelFileStr, float angleLimit, bool forceAutoUV) {
	LoadSingleMesh(meshComponent, ToStdStringPath(modelFileStr), angleLimit, forceAutoUV);
}

void ModelLoader::LoadSingleMesh(UProceduralMeshComponent* meshComponent, const std::string& modelFileStr, float angleLimit, bool forceAutoUV) {
    int i = 0;
    ...
    {
		if (!ReadBinary(data, modelFileStr))return;
        return Read3dsData(meshComponent, data, angleLimit, forceAutoUV);
    }
    else if (stringLowerCase(fext) == "obj")
    {
		if (!ReadUTF8Text(text, modelFileStr))return;
		//UE_LOG(LogTemp, Log, TEXT("%s"),text.c_str());
        return ReadObjText(meshComponent, text, angleLimit, forceAutoUV);
    }
    else if (stringLowerCase(fext) == "wr" || stringLowerCase(fext) == "wrz")
    {
		if (!ReadBinary(data, modelFileStr))return;
		LoadVrmlFile(modelFileStr, data, angleLimit, meshComponent);
    }
    else
    {
...
bool ModelLoader::ReadBinary(std::vector<unsigned char>& outData, const std::string& modelFileStr) {
...
bool ModelLoader::ReadUTF8Text(std::string& outText, const std::string& modelFileStr) {
...
UTexture2D* ModelLoader::LoadTexture2D(const FString& FilePath)
{
...
void ModelLoader::Read3dsData(UProceduralMeshComponent* meshComponent, const std::vector<unsigned char>& ulbin, float angleLimit, bool forceAutoUV)
{
	...
	MeshUtil::UpdateToUProcdualMesh(meshComponent,rawMesh);
}

void ModelLoader::ReadObjText(UProceduralMeshComponent* meshComponent, const std::string& str0, float angleLimit, bool forceAutoUV)
{
...
void ModelLoader::ClearAttached(AActor* v0) {
...
void ModelLoader::LoadVrmlFileToActor(AActor* v0, const std::string& fileStr0, float defaultAngleLimit, UMaterialInterface* baseMaterial, UMaterialInterface* transparentMaterial, ModelLoaderSetCameraHandler onSetCamera, bool forceAutoUV)
{
	...
	UProceduralMeshComponent* comp = v0->FindComponentByClass<UProceduralMeshComponent>();
	if (comp != nullptr) {
...
void ModelLoader::LoadVrmlFile(const std::string& fileStr0, std::vector<unsigned char>& ulstr0, float defaultAngleLimit, UProceduralMeshComponent* destSingleMesh, AActor* v0, UMaterialInterface* baseMaterial, UMaterialInterface* transparentMaterial, bool forceAutoUV, ModelLoaderSetCameraHandler onSetCamera, bool isInline) {
	...
	std::map<std::string, FProcMeshSection*> definedMesh;
	...
	for (i = 0; i < leng0; i++) {
				...
				if (v0 != nullptr) {
					if (str != "Shape" || shapeCountInTransform > 1 || vt==v0) {
						if (shapeCount > 0 || hasCamera || hasInlineRef || isInline) {
							...
							//vt->GetRootComponent()->SetMobility(EComponentMobility::Movable);

							vt = v0->GetWorld()->SpawnActor<AActor>(FVector::ZeroVector, FRotator::ZeroRotator);
							UProceduralMeshComponent*comp1= NewObject<UProceduralMeshComponent>(vt);
							vt->SetRootComponent(comp1);
							comp1->SetMobility(EComponentMobility::Movable);
					...
					meshComp = vt->FindComponentByClass<UProceduralMeshComponent>();
		...
		if (ulstri >= 'A' && ulstri <= 'Z' || ulstri >= 'a' && ulstri <= 'z' || ulstri == '_') {
			...
			if (str == "geometry" && stackType[stackType.size() - 1] == "Shape" && getNextWord(ulstr, i, 1) == "USE" && vt != nullptr) {
				...
				if (definedMesh.find(key) != definedMesh.end()) {
					...
					UProceduralMeshComponent* mf=vt->FindComponentByClass<UProceduralMeshComponent>();
					mf->ClearAllMeshSections();
					mf->SetProcMeshSection(0, *definedMesh[key]);
					useSharedMesh = true;
				}
			}
			...
			if (str == "size" && stackType[stackType.size() - 1] == "Box") {
				...
				MeshUtil::UpdateToUProcdualMesh(meshComp, rawMesh1);
				isPreset = true;
			}
			if (str == "radius" && stackType[stackType.size() - 1] == "Sphere") {
				getNumber(ulstr, infoArr, 1, i);
				MeshUtil::UpdateToUProcdualMesh(meshComp, UModel3d::Sphere(infoArr[0])->m_rawMesh);
				isPreset = true;
			}
			if (str == "bottomRadius" && stackType[stackType.size() - 1] == "Cone") {
				...
				if (!std::isnan(rgeo) && !std::isnan(hgeo)) {
					MeshUtil::UpdateToUProcdualMesh(meshComp, UModel3d::Cone(rgeo,hgeo)->m_rawMesh);
					isPreset = true;
				}
			}
		...
		if (ulstri == '}' && stackType.size() > 0) {
			if (stackType[stackType.size() - 1] == "Shape") {
				...
				else if (meshComp!=nullptr && rawMesh.VertexPositions.Num() > 0 && rawMesh.WedgeIndices.Num() > 0)
				{
					...
					MeshUtil::UpdateToUProcdualMesh(meshComp, rawMesh);

					if (definedGeometryKey != "")
					{
						definedMesh[definedGeometryKey] = meshComp->GetProcMeshSection(0);
					}
					//if (meshComp != nullptr) {
	...
	if (!isInline) {
		...
		else if (shapeCount == 1 && !hasInlineRef && destSingleMesh!=nullptr && destSingleMesh!=meshComp && rawMesh.VertexPositions.Num() > 0 && rawMesh.WedgeIndices.Num() > 0) {
			MeshUtil::UpdateToUProcdualMesh(meshComp, rawMesh);
		}
		else {
...
std::string ModelLoader::getPrevWord(std::vector<unsigned char>& ulstr, int i0, int wordNumber)
{
	int i = 0;
	int j = 0;
...
std::string ModelLoader::getNextWord(std::vector<unsigned char>& ulstr, int i0, int wordNumber)
{
	int i = 0;
	int j = 0;
	bool readBegin = false;
...
int ModelLoader::getNumber(std::vector<unsigned char>& ulstr, std::vector<float>& nums, int numCount, int i0)
{
	int i = 0;
	int j = 0;
	int k = 0;
...
int ModelLoader::getNumberTable(std::vector<unsigned char>& ulstr, std::vector<std::vector<float>>& nums, int columnCount, bool useSeparatorM1, int i0)
{
	int i = 0;
	int j = 0;
	bool readBegin = false;
...
std::string ModelLoader::GetAbsolutePath(const std::string& relativePath0, const std::string& filePathNameBase0)
{
...
FVector ModelLoader::AxisRotationToEularAngles(float x0, float y0, float z0, float w0)
{
...
FVector ModelLoader::vrmlAxisRotationToEularAngles(float x0, float y0, float z0, float w0) {
